...
bio breaks in the WC (Sanity; Pat can't scold you in there), reading silly corporate emails pinned on the break-room
noticeboard (47, no repeats until all are read), chatting with colleagues, playing quiz mini-games with them (Code It!,
Abstract Blitz, Serious or Not?, Spot the Error), table tennis, WHACK-A-SIGNAL on the break-room TV, and
...
Cases arrive in waves (`CONFIG.cases.waveSeconds/waveAmp`): busy stretches, then quiet lulls, and new cases wait while
`maxWaiting` on-time cases are already queued. Later days press through faster stat drain (`CONFIG.difficulty`) rather
than a nonstop case stream, so there is always time for coffee, snacks, chats and mini-games.
...
## Colleague quizzes (same engine as PV Trivia)
- Banks: `CONTENT.minigames.<meddra|lit|serious|qc>.bank` (40 each), placed right after the trivia bank. Same item format;
  `type: 'mc'` (2-4 options, shuffled) or `'bin'` (two fixed buttons, e.g. Serious / Non-serious). Spot the Error items
  leave `q` empty and use the quiz `prompt`. Ids are globally unique (`cq-med-…`, `cq-lit-…`, `cq-ser-…`, `cq-qc-…`).
- Each quiz has its own no-repeat bag (`drugSafetySim.quizBag.<id>`) and its own streak (`S.quizStreaks`).
- Tuning: `CONFIG.minigames.quizRounds`, `quizSeconds` (per type), `quizReward`.

## Whack-a-Signal (break-room TV)
- 30-second round on a 3x3 wall of monitors: whack red signals, leave grey noise. Click/tap, numpad-layout 1-9,
  or arrows/WASD/joystick + E. Labels in `CONTENT.minigames.whack` (pixel font: caps, digits, basic punctuation),
  tuning and rewards in `CONFIG.minigames.whack`.
...
- Fictional content only: no real patient data, product names, colleague names, logos or confidential info.
  No real company names anywhere (not even the employer's name in the footer).
...
mini-games (`startMinigame`, `PongGame`) · question games (`makeQuestionBag`, `TriviaBag`, `quizBag`, `TriviaGame` for
trivia + colleague quizzes), `WhackGame`, noticeboard (`MailBag`) · leaderboard (`Board`) · UI · input · main loop.
Mini-game text lives in `CONTENT.minigames`; trivia questions in `CONTENT.triviaBank`; emails in `CONTENT.mails`.
...
node smoke.js        # real input end-to-end: walk, coffee, talk, work a case, late case -> scolding, director,
                     # events, meeting, quiz, table tennis, Whack-a-Signal, noticeboard emails, name entry, leaderboard (mocked + offline fallback + live read-only check),
...
node trivia.js       # trivia bank integrity + counts per category/difficulty, no-repeat bag (150 draws, reshuffle,
                     # persistence), difficulty ramp, screenshots of every question format, colleague quiz banks + their bags
